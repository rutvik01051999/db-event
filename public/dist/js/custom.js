function togglePassword(id, element) {
  ...
  $(function () {
    ...
    $(".form-select2").on('select2:select', function (e) {
        let select = $(this);
        let allowClear = $('<span class="select2-selection__clear">×</span>');
        $(this).next().find('.select2-search__field').focus();
        $(this).next().find('.select2-selection__rendered').prepend(allowClear);
        allowClear.on('click', function() {
            select.val([]).change();
        });
    }).on('select2:unselect', function (e) {
            let select = $(this); console.log(select.val());
            let allowClear = $('<span class="select2-selection__clear">×</span>');
