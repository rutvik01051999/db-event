@if (!(\Carbon\Carbon::parse($event->start_date)->format('Y-m-d') < \Carbon\Carbon::now()->format('Y-m-d')))
    <div class="d-flex align-items-center">
        ...
        <a class="text-secondary mx-2" href="{{ route('event.question.list', ['id' => $event->id]) }}">
        ...
        <a class="text-warning mx-2 download-qr-code" href="{{ route('event.download-qr-code', $event->id) }}">
            <i class="fa fs-18 fa-regular fa-qrcode"></i>
        </a>
        @if ($event->status == 1)
